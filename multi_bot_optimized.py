#!/usr/bin/env python3
"""
Multi-Bot Optimizado - Gestiona mejor las solicitudes a Gemma2
"""
...
class OptimizedMultiBotSystem:
    def __init__(self):
        """Inicializar sistema multi-bot optimizado"""
        self.ollama_url = "http://localhost:11434"
        self.model = "gemma2:2b"
        
        # Cola para gestionar solicitudes a Gemma2
        self.gemma_queue = queue.Queue()
        self.gemma_worker_thread = None
        self.running = True
    ...
    def gemma_worker(self):
        ...
        while self.running:
            try:
                ...
                bot_name, prompt, personality, callback = request
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        callback(bot_name, message_content, None)
                    else:
                        callback(bot_name, None, f"Error Gemma2: {response.status_code}")
                
                except Exception as e:
                    callback(bot_name, None, f"Error: {e}")
    ...
    def get_prompt_for_personality(self, personality):
        ...
        prompts = {
            "friendly": [
                ...
                "Qué tal si animamos al grupo con algo bueno"
            ],
            "casual": [
    ...
    def gemma_callback(self, bot_name, message_content, error):
        ...
        if error:
            ...
            if bot and message_content:
                ...
                if self.send_message_to_zulip(bot, channel, message_content):
                    ...
                    print(f"  {bot_name}: Mensaje #{self.message_counts[bot_name]} enviado a #{channel}")
                else:
                    self.error_counts[bot_name] += 1
                    print(f"  {bot_name}: Error al enviar mensaje")
    ...
    def run_bot(self, bot):
        ...
        while self.running:
            try:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] {bot['name']} solicitando mensaje...")
                
                # Agregar solicitud a la cola de Gemma2
                self.gemma_queue.put((bot["name"], prompt, bot["personality"], self.gemma_callback))
    ...
    def start_all_bots(self):
        ...
        print("=== SISTEMA MULTI-BOT OPTIMIZADO ===")
        print(f"Bots activos: {len(self.bots)}")
        print(f"Modelo: {self.model}")
        print("Presiona Ctrl+C para detener todos los bots")
        print()
        
        # Iniciar worker de Gemma2
        self.gemma_worker_thread = threading.Thread(target=self.gemma_worker)
        self.gemma_worker_thread.daemon = True
        self.gemma_worker_thread.start()
        ...
        try:
            ...
            if self.gemma_worker_thread:
                self.gemma_worker_thread.join(timeout=5)
...
def main():
    """Función principal"""
    system = OptimizedMultiBotSystem()
